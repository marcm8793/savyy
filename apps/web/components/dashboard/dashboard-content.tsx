...
export function DashboardContent() {
  const { data: accounts, isLoading: accountsLoading } = trpc.account.getAccountsFromDb.useQuery({
    limit: 100,
    offset: 0,
  });
  ...
  const { data: transactionStats, isLoading: statsLoading } = trpc.transaction.stats.useQuery({
    dateRange: {
  ...
  const totalWealth = accounts?.reduce((sum, account) => {
  ...
  const monthlySpendingData = React.useMemo(() => {
    ...
    recentTransactions.transactions.forEach(transaction => {
      ...
      const amount = transaction.amount ? Number(transaction.amount) / Math.pow(10, transaction.amountScale || 0) : 0;
      
      if (amount < 0) { // Only expenses (negative amounts)
        if (!monthlyData[monthKey]) {
          monthlyData[monthKey] = { amount: 0, date: new Date(date.getFullYear(), date.getMonth(), 1) };
        }
    ...
    return Object.entries(monthlyData)
      .map(([month, data]) => ({ month, spending: data.amount, date: data.date }))
  ...
  const accountDistributionData = accounts?.map((account, index) => {
    // Balance in database is stored as cents (multiplied by 100), so divide by 100
    const balance = account.balance ? Number(account.balance) / 100 : 0;
    return {
  ...
  const chartConfig = {
    ...
    ...accountDistributionData.reduce((acc, account) => {
      ...
      const displayName = account.name.length > 15 
  ...
  const averageDailySpending = totalExpenses / 90; // Last 90 days (3 months) average
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <AccountDistributionChart data={accountDistributionData} chartConfig={chartConfig} />
      </div>
